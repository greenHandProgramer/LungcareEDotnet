...
namespace LungCare.SupportPlatform.UI.Windows.Examination
{
    ...
    /// </summary>
    public partial class ImportDicomWindow2 : Window
    {


        public ImportDicomWindow2()
        {
            InitializeComponent();
            LoadComponent();
        }

        public ImportDicomWindow2(string dicomPath)
        {
            InitializeComponent();
            LoadComponent();
            if (_dictionary != null)
        ...
        private void LoadComponent() {
            ...
            panel.Controls.Add(btnWidthAndLevel);
            ...
            p1.SetToolTip(this.btnSaveCT, "上传");
...
文件保存到{0}。", fileName));
        ...
        private RibbonStyle.RibbonMenuButton addThumbImageRibbonBtn(String CTFolderPath,int numb)
        {
            if (!_dictionary.ContainsKey(CTFolderPath))
            {
                return null;
            }

            if (_dictionary[CTFolderPath] != null)
            {
                if (_dictionary[CTFolderPath].Count == 0)
                {
                    return null;
                }
            }
            ...
            try
            {
                image = new DicomImage(Path.Combine(CTFolderPath, CTFileList[0].ToString()));
        ...
        private void btnTemp_Click(object sender, EventArgs e)
        {
            ...
            if (number < _CTPathList.Count)
            {
                ...
                ctview.SetData(_selectedtCTDicomInfo, _dictionary[_selectPath].Count);
            }
            else {
        ...
        private void getPatientData(String filePath) {
        ...
        void btnImportCT_Click(object sender, EventArgs e)
        {
            ...
            if (fbd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                if (_CTPatientsList != null)
                {
                    _CTPatientsList.Clear();
                }
                panelCTList.Controls.Clear();
                _index = 0;
            ...
            //    RibbonStyle.RibbonMenuButton btnTemp = addThumbImageRibbonBtn(path,i);
        ...
        private void setCTData(String dicomPath)
        {
            ...
            foreach (string file in files)
            {
                ...
                if (isDicom(file))
                {
                    ...
                    if (tempIndex==0)
                    {
                        ...
                        RibbonStyle.RibbonMenuButton btnTemp = addThumbImageRibbonBtn(path, _index);
                        panelCTList.Controls.Add(btnTemp);
                        setCTData(file);
                        ...
                        ++_index;
                        firstDicom = false;
                        _selectedtCTDicomInfo = (CTDicomInfo)_CTPatientsList[tempIndex];

                        if (btnTemp != null)
                        {
                            btnTemp.PerformClick();
                        }
                        ctview.SetData(_selectedtCTDicomInfo, files.Length);
